...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs, newsletter }) => {
        ...
        // Use newsletter-specific paths for newsletter uploads
        const pathMap = newsletter ? NEWSLETTER_MEDIA_PATH_MAP : MEDIA_PATH_MAP;
        const uploadPath = pathMap[mediaType] || MEDIA_PATH_MAP[mediaType];
        for (const { hostname } of hosts) {
            logger.debug(`uploading to "${hostname}"${newsletter ? ' (newsletter)' : ''}`);
            const auth = encodeURIComponent(uploadInfo.auth);
            // Add server_thumb_gen=1 for newsletter uploads
            const thumbGenParam = newsletter ? '&server_thumb_gen=1' : '';
            const url = `https://${hostname}${uploadPath}/${fileEncSha256B64}?auth=${auth}&token=${fileEncSha256B64}${thumbGenParam}`;
            let result;
            try {
                ...
                if (result?.url || result?.direct_path) {
                    urls = {
                        ...
                        fbid: result.fbid,
                        ts: result.ts,
                        // Lia@Changes --- capture newsletter thumbnail info returned by the server (server_thumb_gen=1)

    ...
    const relayMessage = async (
	    ...
	    await authState.keys.transaction(async () => {
	    	...
	    	if (isNewsletter) {
	    		...
	    		// Lia@Changes 02-02-26 --- push additionalNodes for newsletter too
	    		if (additionalNodes && additionalNodes.length > 0) {
		    		binaryNodeContent.push(...additionalNodes)
		    	}
	    		// Lia@Changes 02-02-26 --- add extraAttrs (mediatype etc.) to plaintext node — required for media to be accepted in newsletters
            ...
            if (typeof content === 'object' &&
                ...
                if (messageType) {
                    switch(messageType) {
                        ...
                        case 'ALBUM_V2':
                            return await luki.handleAlbumV2(content, jid, userJid, options)
                        case 'ALBUM':
